...
class ModelPredictiveControllerHybrid():
    ...
    def compute_sparse_control(self, device, time_now: Optional[float] = None):
        """ Run sparse model inference once. Does not output control.
        """
        # time alignment check (only uses caller's time_now)
        if time_now is not None:
            for rid in self.id_list:
                dt_rgb = time_now - self.sparse_obs_last_timestamps[f"rgb_time_stamps_{rid}"]
                dt_ts_pose = time_now - self.sparse_obs_last_timestamps[f"robot_time_stamps_{rid}"]
                dt_wrench = time_now - self.sparse_obs_last_timestamps[f"wrench_time_stamps_{rid}"]
                print(
                    f"[MPC] obs lagging for robot {rid}: "
                    f"dt_rgb: {dt_rgb}, dt_ts_pose: {dt_ts_pose}, dt_wrench: {dt_wrench}"
                )
    ...
    def update_control(self, time_step, device, time_now: Optional[float] = None,):
        ...
        if time_step > self.horizon_start_time_step + self.sparse_execution_horizon_time_step:
            ...
            self.horizon_start_time_step = time_step

            print(
                "[MPC debug] new horizon start time step: ",
                self.horizon_start_time_step,)
            return None


        time_now_in_horizon = time_step - self.horizon_start_time_step
        print("[MPC debug] time_step in horizon: ", time_now_in_horizon)
        time_now_in_horizon = max(time_now_in_horizon, 0)
        target_now = self.sparse_action_traj(time_now_in_horizon)
        if (np.linalg.norm(target_now[1].reshape([4, 4])[:3, 3]) < 1e-3):
            print('Warning: sparse action is zero at time_now:', time_now_in_horizon)
        return target_now
